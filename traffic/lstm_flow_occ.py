...
FEATURE = ['Av_Speed','Av_Occupancy']
MAIN_FEATURE = 'Av_Speed'
...
def multi_supervised(dataset):
    df_shift = dataset[MAIN_FEATURE].shift(-1)
    sumdf = pd.concat([dataset[FEATURE],df_shift], axis=1)
    sumdf.fillna(0, inplace=True)
#    result= sumdf.values
#    sumdf = np.array(sumdf)
    return sumdf
...
raw = multi_supervised(df_time)
raw= raw.values

from sklearn.model_selection import train_test_split
X_train, X_test, y_train, y_test = train_test_split(raw[:,:-1], raw[:,-1], test_size = 0.2 ,shuffle = False, stratify = None)
...
X_train_scaled = np.reshape(X_train_scaled, (-1,TIMESTEP,len(FEATURE))) #未知列數,5代表timestep,1代表維度
y_train_scaled = np.reshape(y_train_scaled, (-1,TIMESTEP))

#X_test_scaled = np.reshape(X_test,(-1,1))
X_test_scaled = sc.transform(X_test)
X_test_scaled = np.reshape(X_test_scaled, (-1, TIMESTEP, len(FEATURE)))
...
print("Test_ MAPE:{:.6f}".format(mape))
